...
def autoLogin(url):
    ...
    wait.until(EC.visibility_of_element_located(
        (By.ID, 'smsPwd'))).send_keys(password)
    browser.find_element(By.ID, 'sendsms_btn').click()
    # browser.execute_script('jkManageSMS()')
    print('输入验证码后点击登录。\n此操作等待 5 分钟。')

    browser.execute_script(
        "arguments[0].focus();", browser.find_element(By.ID, 'dynamic_smsPwd'))
...
def openNode(browser, node_name='CMNET城域网'):
    ...
    menuEle = browser.find_element(
        By.CSS_SELECTOR, 'div#resgroupTree table:nth-child(1)')
    selector = 'tr:nth-child(1) td:nth-child(1)'

    """点击 地市公司"""
    cityEle = menuEle.find_element(
        By.CSS_SELECTOR, 'table:nth-child(1) table:nth-child(1)')
    wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, selector)))
    cityEle.find_element(By.CSS_SELECTOR, selector).click()

    """点击 铁岭公司"""
    tieling = cityEle.find_element(
        By.CSS_SELECTOR, 'tr:nth-child(2) table:nth-child(1)')
    wait.until(EC.visibility_of_element_located((By.CSS_SELECTOR, selector)))
    ...
    browser.find_element(
        By.CSS_SELECTOR, 'div#amsgrid table#amsgrid_headTable tr:nth-child(1) td:nth-child(2)').click()
